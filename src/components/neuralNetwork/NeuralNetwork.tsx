 ...
 */
        ...
        const animate = (timestamp: number) => {
            ...
            updatedNodes.forEach((node, index) => {
                ...
                if (index === 0) {
                    ...
                    activeEvents.forEach((event) => {
                        ...
                        switch (event.type) {
                            ...
                            case 'wave':
                                if (event.origin && event.affectedNodes) {
                                    ...
                                    if (
                                        ...
                                        if (dist > 0) {
                                            node.vx +=
                                                (dx / dist) * 0.05 * waveEffect * event.intensity;
                                            node.vy +=
                                                (dy / dist) * 0.05 * waveEffect * event.intensity;
                                        }
                                    }
                                }
                                break;
                            ...
                            case 'colorShift':
                                node.hue =
                                    (Math.sin(now * 0.001 + index) * 0.5 + 0.5) * 30 * event.intensity;
                                break;
                            case 'seizure':
                                node.vx +=
                                    (Math.random() - 0.5) * 0.8 * event.intensity * deltaMult;
                                node.vy +=
                                    (Math.random() - 0.5) * 0.8 * event.intensity * deltaMult;
                    ...
                    const jitterAmplitude =
                        node.mood === 'calm' ? 0.01 : node.mood === 'excited' ? 0.03 : 0.08;
                    if (Math.random() < 0.3 * deltaMult) {
                    ...
                    const maxVel =
                        node.mood === 'calm' ? 0.15 : node.mood === 'excited' ? 0.3 : 0.5;
                ...
                const oscillationFactor =
                    1 + Math.sin(now * 0.0002 + index * 0.5) * 0.08;
                ...
                if (distFromHub > idealDist * 1.15) {
                    ...
                    distFromHub > idealDist * 1.05 ||
                    distFromHub < idealDist * 0.95
                ) {
                ...
                if (node.x > canvas.width - safeMargin) {
                    node.vx -=
                        edgeForceFactor *
                        (1 - (canvas.width - node.x) / safeMargin);
                ...
                if (node.y > canvas.height - safeMargin) {
                    node.vy -=
                        edgeForceFactor *
                        (1 - (canvas.height - node.y) / safeMargin);
                    if (node.y > canvas.height - 10) node.y = canvas.height - 10;
                }

                const dampingFactor =
                    node.mood === 'calm' ? 0.95 : node.mood === 'excited' ? 0.97 : 0.98;
                ...
                if (node.activationLevel > 0) {
                    const decayMultiplier =
                        node.mood === 'calm' ? 1 : node.mood === 'excited' ? 0.7 : 0.5;
                ...
                node.dendrites.forEach((dendrite) => {
                    ...
                    if (dendrite.branches) {
                        dendrite.branches.forEach((branch) => {
                            branch.activity =
                                branch.activity * 0.9 + dendrite.activity * 0.1;
                        });
                    }
                });
                ...
                node.radius =
                    node.baseRadius * (1 + node.activationLevel * 0.2 + oscillation * 0.05);
            ...
            updatedConnections.forEach((connection, idx) => {
                ...
                activeEvents.forEach((event) => {
                    switch (event.type) {
                        case 'storm':
                            ...
                            break;
                        case 'seizure':
                            connection.currentWidth =
                                connection.width *
                                (1 + Math.sin(now * 0.01 + idx) * event.intensity);
                            break;
                        case 'colorShift':
                ...
                if (
                    sourceNode.activationLevel > sourceNode.threshold &&
                    ...
                    if (Math.random() < sourceNode.activationLevel * 0.8) {
                        ...
                        connection.pulseSize =
                            0.8 + sourceNode.activationLevel * 0.5 + Math.random() * 0.2;
                ...
                const activeFactor =
                    timeSinceLastPulse < 1000 ? (1 - timeSinceLastPulse / 1000) * 2 : 0;
                    ...
                    for (let i = 0; i < connection.pulsePositions.length; i++) {
                        ...
                        if (connection.pulsePositions[i] >= 1) {
                            const targetNode = updatedNodes.find(
                                (n) => n.id === connection.target.id
                            );
                            if (targetNode) {
                                if (connection.isInhibitory) {
                                    targetNode.activationLevel = Math.max(
                                        0,
                                        targetNode.activationLevel -
                                        connection.strength * 0.2 * connection.health
                                    );
                                } else {
                                    targetNode.activationLevel = Math.min(
                                        1,
                                        targetNode.activationLevel +
                                        connection.strength * 0.3 * connection.health
                                    );
                                }
                            }
                            ...
                            if (connection.pulsePositions.length === 0) {
                                ...
                                // NEW FEATURE: Optionally play short beep on pulse end
                                /*
                                const audio = new Audio('/audio/pulse.wav');
                                audio.volume = 0.05;
                                audio.play();
                                */
                            }
                ...
                const spontChance =
                    0.0005 * networkActivity * networkActivity * connection.health;
            ...
            updatedNodes.forEach((node) => {
                node.dendrites.forEach((dendrite) => {
                    ...
                    const ctrlX =
                        startX +
                        Math.cos(dendrite.angle + dendrite.curve) * dendrite.length * 0.5;
                    const ctrlY =
                        startY +
                        Math.sin(dendrite.angle + dendrite.curve) * dendrite.length * 0.5;
                    ...
                    if (dendrite.branches) {
                        dendrite.branches.forEach((branch) => {
                            ...
                            const branchEndX =
                                endX + Math.cos(branch.angle) * branch.length;
                            const branchEndY =
                                endY + Math.sin(branch.angle) * branch.length;
                            const branchCtrlX =
                                endX +
                                Math.cos(branch.angle + branch.curve) *
                                branch.length *
                                0.5;
                            const branchCtrlY =
                                endY +
                                Math.sin(branch.angle + branch.curve) *
                                branch.length *
                                0.5;
                            ...
                            const branchColor =
                                branch.activity > 0.1 ? activeColor : baseColor;
            ...
            updatedConnections.forEach((connection) => {
                ...
                connection.pulsePositions.forEach((position) => {
                    if (position >= 0 && position < 1) {
                        ...
                        const subPosition =
                            position * (connection.points.length - 1) - index;

                        const pulseX =
                            connection.points[index].x +
                            (connection.points[nextIndex].x - connection.points[index].x) *
                            subPosition;
                        const pulseY =
                            connection.points[index].y +
                            (connection.points[nextIndex].y - connection.points[index].y) *
                            subPosition;

                        ctx.beginPath();
                        const pulseSize =
                            (2 + connection.strength * 2) * connection.pulseSize;
                        ctx.arc(pulseX, pulseY, pulseSize, 0, Math.PI * 2);

                        // NEW FEATURE: Chromatic aberration effect for strong pulses
                        const aberration = connection.pulseSize > 1.2 ? 5 : 0;
                        if (aberration > 0) {
                            // slight offset circles in R and B
                            ctx.save();
                            ctx.globalCompositeOperation = 'screen';
                            ctx.fillStyle = `rgba(255,0,0,0.2)`;
                            ctx.beginPath();
                            ctx.arc(
                                pulseX - aberration * 0.5,
                                pulseY - aberration * 0.5,
                                pulseSize,
                                0,
                                Math.PI * 2
                            );
                        ...
                        const pulseVariation =
                            Math.sin(now * 0.01 + pulseX * 0.01) * 20;
                        if (connection.isInhibitory) {
                            ctx.fillStyle = `rgba(${210 + pulseVariation}, 180, ${220 - pulseVariation
                                }, 0.9)`;
                        } else {
                            ctx.fillStyle = `rgba(${230 + pulseVariation}, ${210 - pulseVariation
                                }, 180, 0.9)`;
                        }
                        ctx.fill();
            ...
            updatedNodes.forEach((node) => {
                ...
                if (node.mood === 'erratic' && node.activationLevel > 0.3) {
                    ...
                    for (let i = 0; i <= segmentCount; i++) {
                        ...
                        const radiusVariation =
                            node.radius *
                            (1 + deformFactor * Math.sin(angle * 3 + now * 0.01));
                ...
                cellGradient.addColorStop(
                    1,
                    baseColor.replace(', 0.8)', ', 0.7)')
                );
                ctx.fillStyle = cellGradient;
                ctx.fill();

                ctx.strokeStyle =
                    node.activationLevel > 0.3
                ...
                if (node.mood === 'erratic' && node.activationLevel > 0.4) {
                    ...
                    for (let i = 0; i <= segmentCount; i++) {
                        ...
                        const radiusVariation =
                            nucleusRadius *
                            (1 + irregularity * Math.sin(angle * 2 + now * 0.008));
                        const x = node.x + Math.cos(angle) * radiusVariation;
                        const y = node.y + Math.sin(angle) * radiusVariation;
                        if (i === 0) {
                            ctx.moveTo(x, y);
                ...
                if (node.mood === 'excited') {
                    nucleusColor1 =
                        node.activationLevel > 0.3
                            ? 'rgba(220, 205, 190, 0.95)'
                            : 'rgba(205, 195, 180, 0.9)';
                    nucleusColor2 =
                        node.activationLevel > 0.3
                            ? 'rgba(200, 185, 170, 0.8)'
                            : 'rgba(180, 170, 155, 0.7)';
                } else if (node.mood === 'erratic') {
                    const pulseOffset = Math.sin(now * 0.003) * 10;
                    nucleusColor1 =
                        node.activationLevel > 0.3
                            ? `rgba(${210 + pulseOffset}, ${200 - pulseOffset}, 185, 0.95)`
                            : 'rgba(200, 190, 180, 0.9)';
                    nucleusColor2 =
                        node.activationLevel > 0.3
                            ? `rgba(${190 + pulseOffset}, ${180 - pulseOffset}, 165, 0.8)`
                            : 'rgba(170, 160, 150, 0.7)';
                } else {
                    nucleusColor1 =
                        node.activationLevel > 0.3
                            ? 'rgba(210, 200, 190, 0.95)'
                            : 'rgba(200, 190, 180, 0.9)';
                    nucleusColor2 =
                        node.activationLevel > 0.3
                            ? 'rgba(190, 180, 170, 0.8)'
                            : 'rgba(170, 160, 150, 0.7)';
                }
            ...
            activeEvents.forEach((event) => {
                if (event.type === 'wave' && event.origin) {
                    ctx.beginPath();
                    const radius =
                        Math.min(canvas.width, canvas.height) * event.progress * 0.8;
                    ...
                    waveGradient.addColorStop(
                        0.7,
                        `rgba(200, 180, 160, ${alphaFactor * 0.1})`
                    );
    ...
    }, [nodes, connections, hoveredNode, networkActivity, networkEvents, particles, sparks]);
            ...
            for (const node of nodes) {
                ...
                if (dist <= hitRadius) {
                    ...
                    if (node.activationLevel < 0.4) {
                        ...
                        if (hoveredNode) {
                            hoveredNode.activationLevel = Math.min(
                                0.6,
                                hoveredNode.activationLevel + 0.03
                            );
            ...
            if (hoveredNode) {
                ...
                if (clickedNode) {
                    ...
                    updatedNodes.forEach((node) => {
                        if (node.id !== clickedNode.id) {
                            ...
                            if (dist < MAX_REACH) {
                                ...
                                node.activationLevel = Math.min(
                                    1,
                                    node.activationLevel + influence
                                );
                            }
                        }
                    });
                    ...
                    const outgoingConnections = connections.filter(
                        (c) => c.source.id === clickedNode.id
                    );
                    const updatedConnections = [...connections];
                    outgoingConnections.forEach((axon) => {
                        const connectionToUpdate = updatedConnections.find(
                            (c) =>
                                c.source.id === axon.source.id &&
                                c.target.id === axon.target.id
                        );
    ...
    const getNetworkMetrics = () => {
        ...
        const avgHealth =
            connections.length > 0
                ? Math.round(
                    (connections.reduce((sum, c) => sum + c.health, 0) / connections.length) * 100
                )
                : 0;
        const moodCounts = {
        ...
        const currentEvent =
            networkEvents.length > 0 ? networkEvents[0].type.toUpperCase() : 'STABLE';
                ...
                <div className="metrics-container">
                    ...
                    <div className="metric-item">
                        ...
                        <span className="metric-value">
                            {metrics.activeNeurons}/{metrics.totalNodes}
                        </span>
                    </div>
                    <div className="metric-item">
                    ...
                    <div className="metric-item">
                        ...
                        <span
                            className={`metric-value state-${metrics.currentEvent.toLowerCase()}`}
                        >
                    ...
                    <div className="metric-item">
                        ...
                        <span
                            className={`metric-value mood-${metrics.dominantMood.toLowerCase()}`}
                        >
